...
bool hasCloseParent(map<string,int> dict, map<string,int>::iterator aim){
	for (map<string,int>::iterator it=dict.begin(); it!=dict.end(); it++){
		if(inTrans(aim->first, it->first) && (aim->first != it->first) && (aim->second == it->second)){
			return true;
		}
	}
	return false;
